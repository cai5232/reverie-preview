...
【朋友圈协议】
当你判断确实应该替用户发布一条朋友圈时，在正常回复末尾追加一段机器标记，格式必须严格为：[MOMENT]朋友圈正文[/MOMENT]。标记中的内容只写朋友圈正文，不要写作者、时间、解释或 JSON。不要在聊天正文中解释这个标记；前端会自动把它保存到朋友圈并从聊天显示中移除。若不需要发布朋友圈，不要输出该标记。`
...
function openRecentEntry(id){
  ...
  list.innerHTML='<article class="mail-detail"><h1>'+liveStateEscape(p.subject||item.subject||'未命名')+'</h1><div class="mail-detail-body">'+detailBody+'</div><div class="mail-detail-meta"><div><b>时间</b><span>'+liveStateEscape(detailDate)+'</span></div><div><b>收件人</b><span>'+liveStateEscape(p.to||'言言')+'</span></div></div></article><nav class="mail-detail-toolbar" aria-label="邮件操作"><button type="button" aria-label="删除"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 7h14M10 4h4l1 3M8 7l1 13h6l1-13M10 10v7M14 10v7" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button><button type="button" aria-label="分享"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 15V3M8 7l4-4 4 4M5 12v8h14v-8" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button><button type="button" aria-label="回复"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a7 7 0 0 0-12.8-3.8L4 10m0 0V5m0 5h5M4 13a7 7 0 0 0 12.8 3.8L20 14m0 0v5m0-5h-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button><button type="button" aria-label="更多"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1.5" fill="currentColor"/><circle cx="12" cy="12" r="1.5" fill="currentColor"/><circle cx="19" cy="12" r="1.5" fill="currentColor"/></svg></button></nav>';
...
let momentSelectedImages=[];
function openMomentReply(button){const main=button&&button.closest('.moment-main');if(!main)return;let box=main.querySelector('.moment-reply-editor');if(!box){box=document.createElement('div');box.className='moment-reply-editor';box.innerHTML='<input type="text" placeholder="回复这条动态…" maxlength="200"><button type="button">发送</button>';box.querySelector('button').onclick=function(){sendMomentReply(this)};main.appendChild(box)}box.hidden=false;box.querySelector('input').focus()}
function sendMomentReply(button){const box=button&&button.closest('.moment-reply-editor'),input=box&&box.querySelector('input'),text=input&&input.value.trim();if(!text)return;const main=box.closest('.moment-main');const reply=document.createElement('div');reply.className='moment-cai moment-reply';reply.textContent='回复：'+text;main.appendChild(reply);box.remove()}
function openMomentComposer(){const s=document.getElementById('momentsComposer');if(!s)return;s.hidden=false;momentSelectedImages=[];const t=document.getElementById('momentComposeText');if(t){t.value='';setTimeout(()=>t.focus(),80)}const g=document.getElementById('momentImageGrid');if(g)g.querySelectorAll('.moment-image-thumb').forEach(x=>x.remove())}
function closeMomentComposer(){const s=document.getElementById('momentsComposer');if(s)s.hidden=true}
function previewMomentImages(event){const grid=document.getElementById('momentImageGrid');if(!grid)return;Array.from(event.target.files||[]).forEach(file=>{const reader=new FileReader();reader.onload=()=>{const src=reader.result;momentSelectedImages.push(src);const img=document.createElement('img');img.className='moment-image-thumb';img.src=src;img.alt='已选图片';img.onclick=()=>openMomentImage(src);grid.insertBefore(img,grid.firstChild)};reader.readAsDataURL(file)});event.target.value=''}
...
function ensureMomentActions(post){
  const main=post.querySelector('.moment-main'); if(!main)return;
  ...
  const oldComment=main.querySelector(':scope > .moment-comment');
  if(oldComment){oldComment.remove();comments.appendChild(oldComment)}
  if(!comments.querySelector('.moment-comment')){
...
function renderStoredMomentComment(post,item){
  const comments=post.querySelector('.moment-comments'); if(!comments)return;
  const row=document.createElement('div');row.className='moment-cai moment-reply moment-comment-item';row.dataset.commentId=item.id;row.dataset.author=item.author;row.setAttribute('role','button');row.textContent=item.replyTo?item.author+'回复'+item.replyTo+'：'+item.text:item.author+'：'+item.text;row.onclick=()=>openMomentReplyFromComment(row);comments.appendChild(row)
}
...
function buildMomentPost(item){
  ...
  const d=new Date(item.createdAt||Date.now());post.querySelector('.moment-date').textContent=d.getFullYear()+'/'+(d.getMonth()+1)+'/'+d.getDate();post.querySelector('.moment-body').textContent=item.text||'';
  const images=post.querySelector('.moment-post-images');(item.images||[]).forEach(src=>{const img=document.createElement('img');img.className='moment-post-image';img.src=src;img.alt='动态图片';img.onclick=()=>openMomentImage(src);images.appendChild(img)});return post
}
function persistCurrentComment(main,text,target){
  const post=main.closest('.moment-post');if(!post)return;
  const author='Koi';
  const postAuthor=main.querySelector('.moment-line h2')?.textContent||'Koi';
  const replyTo=(target&&target!==author&&target!==postAuthor)?target:'';
  const item={id:'c-'+Date.now()+'-'+Math.random().toString(36).slice(2,5),author,replyTo,text};
...
const _createMomentCommentOld=createMomentComment;
createMomentComment=function(main,text,target){persistCurrentComment(main,text,target)}
const _publishMomentOld=publishMoment;
publishMoment=function(){
  ...
  const item={id:'post-'+Date.now(),text,images:momentSelectedImages.slice(),createdAt:new Date().toISOString()};
